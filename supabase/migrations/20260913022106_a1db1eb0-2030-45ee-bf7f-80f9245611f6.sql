...
AS $$
  SELECT p.id, p.nome, p.cliente, p.status, p.etapa, p.etapa_em, p.prazo_entrega,
         ...
         COALESCE(
           NULLIF(p.dados->>'local_instalacao',''),
           NULLIF(concat_ws(', ',
             NULLIF(p.dados->>'cliente_endereco',''),
             NULLIF(p.dados->>'cliente_numero','')
           ) || CASE WHEN COALESCE(p.dados->>'cliente_complemento','') <> ''
                     THEN ' - ' || (p.dados->>'cliente_complemento') ELSE '' END, ''),
           '')
